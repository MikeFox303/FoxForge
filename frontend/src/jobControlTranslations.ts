...
export const jobControlTranslations = {
  en: {
    ...
    sending: 'Sending…',
    uncertain: 'The printer-control outcome is uncertain. FoxForge will not retry automatically. Verify the physical printer and wait for a conclusive job-state or job-identity change before sending another control command.',
  ...
  ru: {
    ...
    sending: 'Отправка…',
    uncertain: 'Результат команды управления неизвестен. FoxForge не будет повторять её автоматически. Проверьте принтер и дождитесь подтверждённого изменения состояния или идентификатора задания перед новой командой.',
  ...
  uk: {
    ...
    sending: 'Надсилання…',
    uncertain: 'Результат команди керування невідомий. FoxForge не повторюватиме її автоматично. Перевірте принтер і дочекайтеся підтвердженої зміни стану або ідентифікатора завдання перед новою командою.',
