...
const bundles = {
  en: {
    alpha: {
      ...
      system: {
        ...
        runtime: 'Runtime', runtimeTitle: 'Alpha runtime', runtimeText: 'The interface reads the live FoxForge API; demo data is available only with ?demo=1.', interfaceRunning: 'Interface running',
      ...
      empty: {
        noPrintersTitle: 'No printers configured', noPrintersText: 'Configure a printer adapter in the FoxForge runtime, then restart the service.',
        noQueueTitle: 'The queue is empty', noQueueText: 'Jobs will appear here when the public command workflow is available.',
        noMaterialsTitle: 'No material systems reported', noMaterialsText: 'Material slots appear after a configured printer reports its capabilities.',
      },
  ...
  ru: {
    alpha: {
      ...
      system: {
        eyebrow: 'Состояние приложения', title: 'Система', text: 'Runtime, развёртывание и настройки интерфейса. Детали разработчика остаются второстепенными.', runtime: 'Runtime', runtimeTitle: 'Альфа-runtime', runtimeText: 'Интерфейс читает живой FoxForge API; демо-данные доступны только с ?demo=1.', interfaceRunning: 'Интерфейс работает', architecture: 'Архитектура', architectureTitle: 'Независимое от производителя ядро', architectureText: 'Backend, frontend и deployment используют стабильные application contracts, а vendor adapters изолированы.', languageText: 'Английский, русский и украинский используют одно дерево компонентов.', diagnostics: 'Диагностика разработчика', frontend: 'Frontend', routing: 'Маршрутизация', serverState: 'Состояние сервера', inventorySource: 'Источник Inventory', backendApi: 'Backend API', realtime: 'Realtime', inventoryLive: 'Живая read model InventoryService', apiConnected: 'Подключён /api/v1', realtimeReserved: 'Сейчас polling; далее WebSocket / SSE',
      },
      ...
      empty: {
        noPrintersTitle: 'Принтеры не настроены', noPrintersText: 'Настройте адаптер принтера в runtime FoxForge, затем перезапустите сервис.',
        noQueueTitle: 'Очередь пуста', noQueueText: 'Задания появятся здесь после подключения публичного сценария команд.',
        noMaterialsTitle: 'Системы материалов не обнаружены', noMaterialsText: 'Слоты появятся, когда настроенный принтер сообщит свои capabilities.',
      },
  ...
  uk: {
    alpha: {
      ...
      empty: {
        noPrintersTitle: 'Принтери не налаштовані', noPrintersText: 'Налаштуйте адаптер принтера в runtime FoxForge, потім перезапустіть сервіс.',
        noQueueTitle: 'Черга порожня', noQueueText: 'Завдання з’являться тут після підключення публічного сценарію команд.',
        noMaterialsTitle: 'Системи матеріалів не виявлені', noMaterialsText: 'Слоти з’являться, коли налаштований принтер повідомить свої capabilities.',
      },
