...
def _encode_spool(spool: Spool) -> str:
    payload = {
        ...
        "empty_spool_mass_g": (
            str(spool.empty_spool_mass_g) if spool.empty_spool_mass_g is not None else None
        ),
...
def _decode_spool(raw: str) -> Spool:
    ...
    return Spool(
        ...
        color=(
            ...
            else SpoolColor(str(payload["color"]))
        ),
        empty_spool_mass_g=(
            None
            if payload.get("empty_spool_mass_g") is None
            else Decimal(str(payload["empty_spool_mass_g"]))
        ),
        purchase_date=(
            None
            if payload.get("purchase_date") is None
            else date.fromisoformat(str(payload["purchase_date"]))
        ),
...
def _load(raw: str, *, object_name: str) -> dict[str, object]:
    ...
    if payload.get("schema_version") != _SCHEMA_VERSION:
        raise ValueError(
            f"unsupported inventory {object_name} schema version: "
