...
The current status is a **first runnable alpha candidate**: backend, live read API, web UI, SQLite queue/inventory persistence and Docker runtime are integrated. Physical printer validation, command APIs, realtime delivery, automatic accounting, farm scheduling and Umbrel packaging remain incomplete.
...
The production alpha UI now consumes live `/api/v1` read models. Demo data remains available only through explicit `?demo=1`. Command mutations and realtime WebSocket/SSE updates are intentionally separate future contracts.
...
- Linux amd64/arm64 publication is prepared, but release-grade ARM64 validation is still required;
...
4. document important architecture/runtime changes in the repository;
5. preserve upstream copyright/license provenance where code or material is derived;
6. avoid claiming physical or production validation until the corresponding tests have actually been run.
