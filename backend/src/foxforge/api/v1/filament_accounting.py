...
def register_filament_accounting_routes(
    ...
    *,
    ...
    """Register operator accounting reads and guarded mutation commands.

    This API does not itself enable automatic provider accounting. Candidate 6
    provider/runtime enforcement remains an explicit R5 composition decision.
    """
    ...
    async def plan(request: web.Request) -> web.Response:
        try:
            ...
            _validate_plan_preflight(accounting, entry, estimates)
            command = _reserve(request, "filament.plan", payload, route_identity=str(queue_id))
        ...
        if not command.created:
            if command.record.state != CommandIdempotencyState.COMPLETED:
    ...
    async def release(request: web.Request) -> web.Response:
        try:
            ...
            command = _reserve(request, "filament.release", payload, route_identity=str(queue_id))
        except QueueEntryNotFoundError:
            return command_error(request, status=404, code="queue_not_found", message="Queue entry was not found.")
        except CommandIdempotencyConflictError as error:
            return command_error(request, status=409, code="idempotency_conflict", message=str(error))
    ...
    async def reconcile(request: web.Request) -> web.Response:
        try:
            ...
            current = _reservation_for_material(accounting, queue_id, material_index)
            _validate_reconciliation_preflight(inventory, current, actual_mass_g)
            command = _reserve(request, "filament.reconcile", payload, route_identity=str(queue_id))
        except QueueEntryNotFoundError:
            return command_error(request, status=404, code="queue_not_found", message="Queue entry was not found.")
            ...
            return command_error(
                request,
                status=409,
                code="filament_spool_unavailable",
                message="The reserved spool no longer exists.",
            )
        except CommandIdempotencyConflictError as error:
            return command_error(request, status=409, code="idempotency_conflict", message=str(error))
        except FilamentCapacityError as error:
            return command_error(request, status=409, code="insufficient_filament", message=str(error))
        except FilamentReconciliationConflictError as error:
            return command_error(request, status=409, code="filament_reconciliation_conflict", message=str(error))
        except FilamentReconciliationRequiredError as error:
            return command_error(request, status=409, code="filament_reconciliation_not_allowed", message=str(error))
        except FilamentSettlementError as error:
            return command_error(request, status=409, code="filament_spool_unavailable", message=str(error))
        except (ValueError, TypeError) as error:
            return command_error(request, status=400, code="invalid_request", message=str(error))
...
def _validate_plan_state(entry: QueueEntry) -> None:
    if (
        ...
        or entry.state not in {QueueEntryState.PENDING, QueueEntryState.BLOCKED, QueueEntryState.FAILED}
    ):
        raise FilamentPlanConflictError("filament plan must be created before the dispatch start boundary")
...
def _validate_release_state(entry: QueueEntry) -> None:
    if (
        ...
        raise FilamentReconciliationRequiredError(
            "only queue entries proven not to have crossed the dispatch start boundary may release reservations"
        )
...
def _validate_plan_preflight(
    ...
    existing = accounting.reservations_for_queue(entry.queue_id)
    if not existing:
        accounting.preview_plan(entry.printer_id, entry.request.material_bindings, estimates)
        return

    if any(item.state != FilamentReservationState.RESERVED for item in existing):
        raise FilamentPlanConflictError(
            "filament plan has already left the reserved state; enqueue a new queue entry to create another plan"
        )

    bindings = {binding.material_index: binding for binding in entry.request.material_bindings}
    estimate_map = {estimate.material_index: estimate for estimate in estimates}
    if len(estimate_map) != len(estimates) or set(bindings) != set(estimate_map):
    ...
    persisted = {item.material_index: (item.estimated_mass_g, item.printer_id, item.slot_id) for item in existing}
    if expected != persisted:
        raise FilamentPlanConflictError("filament plan is immutable once reservations are created")


def _validate_reconciliation_preflight(
    inventory: InventoryService,
    reservation: FilamentReservation,
    actual_mass_g: Decimal,
) -> None:
    if reservation.state == FilamentReservationState.CONSUMED:
        if reservation.actual_mass_g == actual_mass_g:
            return
        raise FilamentReconciliationConflictError("reservation was already reconciled with a different consumed mass")
    if reservation.state == FilamentReservationState.RELEASED:
        if actual_mass_g == 0:
            return
        raise FilamentReconciliationConflictError("reservation was already reconciled/released with zero consumption")
    ...
    spool = inventory.get_spool(reservation.spool_id)
    if spool.archived:
        raise FilamentSettlementError("archived spool cannot receive reconciled material consumption")
    remaining = inventory.balance(reservation.spool_id).remaining_filament_mass_g
    if actual_mass_g > remaining:
        raise FilamentCapacityError(
            f"reserved spool has {remaining} g remaining; cannot reconcile {actual_mass_g} g consumption"
        )
...
def _reservation_for_material(
    ...
    reservation = next(
        (item for item in accounting.reservations_for_queue(queue_id) if item.material_index == material_index),
        None,
    )
